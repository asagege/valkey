...
TEST_F(UtilTest, TestString2ll) {
    ...
    EXPECT_EQ(string2ll(buf, strlen(buf), &v), 0);
    ...
    valkey_strlcpy(buf, "-1", sizeof(buf));
    EXPECT_EQ(string2ll(buf, strlen(buf), &v), 1);
    EXPECT_EQ(v, -1);
...
TEST_F(UtilTest, TestString2l) {
    ...
    /* May not start with +. */
    valkey_strlcpy(buf, "+1", sizeof(buf));
    EXPECT_EQ(string2l(buf, strlen(buf), &v), 0);
    ...
    valkey_strlcpy(buf, "-1", sizeof(buf));
    EXPECT_EQ(string2l(buf, strlen(buf), &v), 1);
    EXPECT_EQ(v, -1);
...
TEST_F(UtilTest, TestFixedpointD2string) {
    ...
    EXPECT_EQ(sz, 19);
    EXPECT_STREQ(buf, "0.01000000000000000");

    v = 10.01;
    sz = fixedpoint_d2string(buf, sizeof buf, v, 4);
    EXPECT_EQ(sz, 7);
    EXPECT_STREQ(buf, "10.0100");
...
TEST_F(UtilTest, TestVersion2num) {
    EXPECT_EQ(version2num("7.2.5"), 0x070205);
    EXPECT_EQ(version2num("255.255.255"), 0xffffff);
    EXPECT_EQ(version2num("7.2.256"), -1);
    EXPECT_EQ(version2num("7.2"), -1);
    EXPECT_EQ(version2num("7.2.1.0"), -1);
    EXPECT_EQ(version2num("1.-2.-3"), -1);
    EXPECT_EQ(version2num("1.2.3-rc4"), -1);
    EXPECT_EQ(version2num(""), -1);
}
...
static int cache_exist(int fd) {
    ...
    EXPECT_NE(m, nullptr);
    EXPECT_EQ(mincore(m, 4096, &flag), 0);
    munmap(m, 4096);
...
TEST_F(UtilTest, TestReclaimFilePageCache) {
    ...
    /* test write file */
    char buf[4] = "foo";
    EXPECT_GT(write(fd, buf, sizeof(buf)), 0);
    EXPECT_TRUE(cache_exist(fd));
    EXPECT_EQ(valkey_fsync(fd), 0);
    EXPECT_EQ(reclaimFilePageCache(fd, 0, 0), 0);
    EXPECT_FALSE(cache_exist(fd));

    /* test read file */
    EXPECT_GT(pread(fd, buf, sizeof(buf), 0), 0);
    EXPECT_TRUE(cache_exist(fd));
    EXPECT_EQ(reclaimFilePageCache(fd, 0, 0), 0);
    EXPECT_FALSE(cache_exist(fd));
...
TEST_F(UtilTest, TestWritePointerWithPadding) {
    ...
    EXPECT_EQ(memcmp(buf, expected, ptr_size), 0);
